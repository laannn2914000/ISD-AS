...
const CreateReport = () => {
  ...
  const handleSubmitReport = async (status = "Submitted") => {
    try {
      ...
      const user = JSON.parse(localStorage.getItem("user")); // Lấy lại user để lấy _id

      const payload = {
        userId: user._id || user.id, // BẮT BUỘC: Để Manager biết báo cáo của ai
        name: formData.title,
        type: template,
        content: formData,
        status: status,
        ...
        createdAt: new Date(), // Thêm ngày tạo để Admin dễ lọc báo cáo mới nhất
      };

      await axios.post("http://localhost:5000/api/reports/create", payload, {
          ...
          {step === 1 ? (
            ...
            <div className="grid grid-cols-12 gap-10">
              ...
              <div className="col-span-8 bg-gray-200 p-8 rounded-[32px] flex justify-center overflow-y-auto max-h-[1200px]">
                <div
                  ref={reportRef}
                  className="bg-white w-[210mm] min-h-[297mm] p-[20mm] shadow-2xl text-black font-serif"
                  style={{ fontSize: "13pt" }}
                >
