    ...
    <div className="flex h-screen bg-[#f8fafc] overflow-hidden font-sans text-left relative">
      ...
      <aside className="w-[280px] bg-yellow-400 border-r border-yellow-500/20 p-6 flex flex-col transition-colors duration-300">
        ...
        <nav className="flex-1 space-y-1">
          ...
          <NavItem icon={<UserCheck size={20} />} label="Phê duyệt báo cáo" onClick={() => navigate("/report-system")} />
...
const NavItem = ({ icon, label, active = false, onClick, disabled = false }) => (
  <div onClick={disabled ? undefined : onClick} className={`flex items-center gap-4 px-4 py-3.5 rounded-xl transition-all ${disabled ? "cursor-not-allowed" : active ? "bg-white text-yellow-500 font-bold shadow-sm" : "text-gray-800 hover:bg-black/10 font-normal cursor-pointer"}`}>
