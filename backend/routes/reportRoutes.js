...
router.get('/my-reports', async (req, res) => {
    try {
        const reports = await Report.find({ creatorId: req.user.id }).sort({ createdAt: -1 });
        res.json(reports);
    } catch (err) {
...
router.get('/search', async (req, res) => {
    try {
        ...
        // Phân quyền: Employee chỉ thấy bài mình, Manager thấy bài theo Dept, Admin thấy hết
        if (req.user.role === "nhân viên") {
            query.creatorId = req.user.id;
        } else if (req.user.role === "manager" || req.user.role === "quản lý") {
            query.dept = req.user.dept; // Manager chỉ thấy báo cáo trong phòng mình
        }
...
router.patch('/:id/decide', async (req, res) => {
    try {
        ...
        if (req.user.role === 'nhân viên') {
            return res.status(403).json({ message: "Bạn không có quyền duyệt báo cáo" });
        }
        ...
        await report.save();
        res.json({ message: `Đã ${action === 'Approve' ? 'Duyệt' : 'Từ chối'} báo cáo`, report });
    } catch (err) {
