...
app.get("/api/manager/stats", verifyToken, async (req, res) => {
  try {
    const total = await User.countDocuments({ managedBy: req.user.id, role: "nhân viên" });
    const locked = await User.countDocuments({ managedBy: req.user.id, role: "nhân viên", isLocked: true });
    res.json({ totalEmployees: total, lockedEmployees: locked, activeEmployees: total - locked });
  } catch (err) { res.status(500).json({ message: "Lỗi thống kê" }); }
});

// 7. ROUTE KHÓA/MỞ KHÓA (ƯU TIÊN LÊN TRƯỚC CÁC ROUTE CÓ :id NGẮN)
app.patch("/api/manager/employees/toggle-lock/:id", verifyToken, async (req, res) => {
...
app.get("/api/manager/employees", verifyToken, async (req, res) => {
  try {
    const employees = await User.find({ managedBy: req.user.id, role: "nhân viên" }).select("-password").sort({ createdAt: -1 });
    res.json(employees);
...
app.post("/api/users/register", verifyToken, async (req, res) => {
  ...
  } catch (err) { res.status(400).json({ message: "Tài khoản đã tồn tại!" }); }
});
...
app.delete("/api/users/:id", verifyToken, async (req, res) => {
  ...
  } catch (err) { res.status(400).json({ message: "Lỗi khi xóa" }); }
});
